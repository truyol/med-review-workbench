...
当前源码完整门禁另行通过：API 31 项、覆盖率 92%；组件 3 项；Playwright 9 项；日志隐私扫描 131 行零命中；依赖审计无已知漏洞。心脏 STL 文件哈希与解析已由单测核对。详细输出见 P8 报告第 12 节。上方干净克隆的 6 条 E2E 是更早的独立验证，不冒称为 9 条，也不冒称为新模型的完整克隆验收。

## 交付时需要如实说明

- 本项目是面试工程原型，不是临床诊断、治疗或医疗器械软件；生成的 phantom/备用曲管不是患者影像，心脏参考 STL 与它们并非同一病例。
