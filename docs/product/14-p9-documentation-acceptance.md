...
| 测试 | 最新完整门禁：API 29（覆盖率 92%）、前端组件 3、Playwright 7（Mock 2 + 真实后端 5）、隐私日志扫描 105 行零命中 | `docs/product/12-p8-test-report.md` 第 3、11 节 |
| 首屏体积 | 当前应用主包约 54KB，vendor 单独拆分；旧 P5 体积数字只属于历史版本 | `README.md`、`docs/product/09-p5-frontend-acceptance.md` |
| 新克隆样例 | Git 内有两张非临床 PNG；缺少外部 DICOM/STL 时 seed 生成非临床 phantom/曲管，原始大样例不入库 | `README.md`、`sample-data/README.md` |
...
- 验收矩阵中的 Playwright 数量改为 7；P5 历史报告标注为阶段记录，并指向最新测试证据；P8 报告不再混用旧轮次的 4/6 条结果。
- 公开仓库目前没有 `LICENSE`。不擅自添加 MIT 或宣称通用复用授权；仓库所有者可另行决定许可证。
