...
## 2026-09-22：默认演示 DICOM 改为仓库内脱敏公开 CT

- 任务：面试官全新克隆时，DICOM 预览是 seed 生成的 64×64 白圆点 phantom，看起来像坏图；要求改为克隆后即可看到真实 CT。
- AI 建议：把 pydicom `CT_small.dcm`（MIT 许可公开测试数据）的清理副本提交进 Git，作为默认 DICOM 演示素材。
- 人工核验：逐标签 dump 清理副本，发现 `OtherPatientIDsSequence` 仍残留 `PatientID='ABCD1234'/'1234ABCD'`、`StationName='CT01_OC0'` 未清；据此把清理脚本升级为 scrub v2（删除直接身份标签、该序列、设备/来源字段与私有标签，并确定性替换 UID），重新生成后复核：无身份标签、无私有块、无序列 ID，应用解析 `ingest_warnings` 为空，且连跑两次哈希一致（`FA9BFB06…8BBD`，34,582 bytes）。
- 修改/拒绝：保留 `synthetic_samples.py` 作为文件缺失时的兜底，但不再称其为默认演示；不提交许可禁止再分发的 Rubo 样例。
- 医疗边界：入库的是公开、已脱敏测试数据；清理脚本仍不等同于临床级去标识化，也不证明像素无烧录文字。

## 2026-09-21：技术设计与面试题文档全量复核

- 任务：按原始全栈面试题复核技术设计、PRD、验收矩阵、白名单、隐私、测试、运维与交付文档，消除英文叙述和“设计即实现”的误读。
- AI 建议：用当前模型、路由、服务、仓储、测试输出重写根技术设计；对 12 字段白名单、旧端点、逐素材来源与评审字段给出明确现状/目标区分。
- 人工核验：逐项读取原题 Word 和仓库代码；运行 Markdown 内部链接检查、完整隔离门禁（API 31/92%、Web 组件 3、Playwright 9、隐私扫描 131 行零命中）；确认心脏 STL 的 Git 入库与许可署名。
- 修改/拒绝：拒绝把原题未强制的独立方案表当成必需缺口；拒绝把通用 `source_label` 说成逐素材溯源、把可空评审备注说成必填结论，或把旧轮次测试数字覆盖历史记录。
- 结果：技术设计改为中文的当前实现契约；PRD/验收/隐私/白名单/阶段门禁与实际代码、证据、延期项对齐，完整门禁仍不代替人工五分钟演示。
...
## 2026-09-21：结构标记配色与尺寸

- 任务：多个结构标记颜色相同、难以区分，且标记点过大。
- AI 建议：每个标记按序使用不同颜色（3D 球与列表点同色）并加序号，同时缩小标记球半径。
- 人工验证：以真实后端写入 3 个标记，断言列表出现 3 种不同颜色与序号 1/2/3。
- 结果：新增共享调色板 `shared/markerColors.ts`；标记球半径由模型半径的 2.5% 降到 1.2%。
- 医疗边界：仅影响工程标记的可见性，不改变评审结论由人工确认的边界。
...
- 验证：修复 Mock E2E 的严格定位冲突并补齐可解码 PNG 预览响应后，完整门禁重新执行通过，最终输出 `Full P8 gate passed.`（当轮 API 25、Web 3、Playwright 4；后续补齐 必须 后为 API 29、Playwright 7）。
...
- AI 建议：把图片比较继续留在 应该；人工对照原始题目后拒绝该建议，因为题目明确要求图片浏览、筛选、比较、整理和结论沉淀，已改为 必须。
...
- 结果：补齐 JSON origins、CORS、统一 404/500 信封、AntD v6 属性、`app/domain/errors.py`；新增图片 必须 验收、真实医疗场景能力差距文档和 AI 决策边界文档。
...
- AI 建议：把 txt 中的“P0/P1 功能范围”转换为本项目的 必须/应该/以后，避免和全局 P0-P10 阶段混淆。
...
- 拒绝“立即接入真实大模型”：不能直接证明核心价值，会增加密钥和可用性风险；改为记录研发 AI 使用，产品内 AI 在本次 MVP 中明确为 不做。
