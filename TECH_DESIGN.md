...
> 实现状态提示（2026-09-21）：本文保留 P2 设计和历史迭代记录，不等同于每项均已落地。当前实现的 API 以根 README 的“API 一览”和运行时 OpenAPI 为准。标签/筛选、图片并排比较和结构标记已实现；`reprocess`、真实鉴权与持久化审计表仍是设计/延期项，不能按本文接口规划误认为已交付。公开仓库默认 seed 使用非临床 DICOM phantom 和 STL 曲管；外部 DICOM/题目 STL 不入 Git。
