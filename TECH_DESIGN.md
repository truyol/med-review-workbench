...
{"decision":"needs_changes","note":"请补充可预览的清理影像。","reviewer_name":"演示评审员"}
```

成功时追加一条 `reviews` 记录，并把素材状态写为 `needs_changes`；`note` 当前可省略。冲突或失败时按统一错误信封返回，例如 `CASE_CODE_CONFLICT` 会携带 `message`、`next_action`、`request_id`，而不是堆栈。
...
- STL：对二进制文件严格核对长度与三角面数；对 ASCII 文件只检查 `solid` 开头和 `facet normal` 特征并统计出现次数，**不是完整语法或几何有效性验证**。损坏时返回 `MODEL_PARSE_FAILED`（422）；目前不计算包围盒、不生成 3D 缩略图。前端通过模型流加载，并提供旋转、缩放、平移、复位与结构标记；加载失败应显示重试提示。
...
日志禁止患者姓名、患者 ID、请求正文、原始文件名及完整 DICOM 标签。仅显示白名单不等同于临床级去标识化；样例 DICOM 清理脚本（scrub v2）也不能证明像素不存在烧录文字，录屏前须人工检查。默认演示 DICOM 是仓库内已脱敏的公开 CT（pydicom `CT_small.dcm` 的 MIT 许可清理副本）；仓库内的心脏参考 STL 是独立模型，**不是**该 CT 或任何病例的重建，不用于临床判断。
...
仍未实现或未验证：真实鉴权/权限控制、`audit_events` 持久化、`reprocess`、完整软删除、逐素材来源/许可/去标识化状态的业务字段、多人协作、PostgreSQL 生产验证及临床合规认证。独立方案对象、方案级评审及医学配准也不在当前范围，但**原题没有强制要求独立方案表**；本项目以病例下素材评审覆盖主要演示路径，不把它说成完整方案管理。权限、审计、隐私合规、专业脱敏和协作的上线前补足方案见 [真实医疗软件能力差距](docs/engineering/real-world-readiness.md)。P2 原计划的若干表字段、端点和错误码已在本版更正为实际代码口径；若以后实现，应连同迁移、测试、OpenAPI 与本设计一起更新，不可仅修改文案。

## 7. 需求与证据追溯

| 需求 | 技术落点 | 当前证据/边界 |
|---|---|---|
| FR-001～004 项目/病例/素材 | 五张表中的前三张、`workbench` 路由/服务/仓储 | 创建/列表/上传/详情与标签备注；FR-002/004 的来源仅有通用类别，逐素材来源/清理状态未做 |
...
| FR-007～009 评审/筛选/比较 | `reviews`、素材列表筛选、前端比较视图 | 评审决定/备注/评审人持久化，问题与下一步未拆字段；比较是前端交互，不另设比较 API |
