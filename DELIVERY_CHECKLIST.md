...
状态：基本完成（仅剩面试官设备复现）。自动化结果、独立克隆和人工演示分别判定，不互相替代。
...
- [x] 人工按 `docs/product/06-demo-script.md` 完整走通五分钟路线（图片比较、DICOM 白名单、STL 四项操作/结构标记、评审与刷新），已录制演示视频随邮件提交。
- [ ] 请面试官在其设备上按 README 复现；邮件已附五分钟演示视频可先行对照（复现尚未发生，故不勾选）。
...
实机复核与人工五分钟演示（已录屏）已完成；仅当面试官设备复现完成后，P10 才能最终标记为“收口完成”。
