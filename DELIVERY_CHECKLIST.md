...
- [x] 公开 GitHub 仓库包含代码、README、PRD、技术设计、运维手册、测试报告、演示脚本、截图与两张非临床合成 PNG。
- [x] Git 不含数据库、上传文件、原始 DICOM/STL、`.env`、虚拟环境或 `node_modules`；全新克隆的 seed 会生成可演示的非临床 DICOM/STL。
- [x] 面试题能力逐项映射和已知范围差异见 `docs/product/13-delivery-readiness.md`，不把原型说成临床系统。
- [x] 根 README 给出面试官可复制的 Compose 启动、seed 和停止命令。
- [x] 最新完整 P8 门禁：API 29 / 92%、组件 3、Playwright 7、隐私日志扫描 105 行零命中、依赖审计通过，详见 `docs/product/12-p8-test-report.md`。

## P10 实机复核

- [x] 从 GitHub `main` 独立克隆（`--depth 1`），只按 README 启动并 seed：隔离栈 `medreview-p10-check`（端口 18082）进入 healthy，seed 得到 1 项目、`DEMO-TAVR-001` 与 PNG/DICOM/STL 三类素材；DICOM 元数据为 `PHANTOM`/64×64（全新克隆的 fallback），预览 HTTP 200、模型流 HTTP 200。
...
- [x] 外部 DICOM/STL 样例不随 Git 再分发；默认 seed 素材为工程合成样例，不是患者数据。
